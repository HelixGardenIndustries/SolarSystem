...
function loadTexture(textureName){
    return new THREE.ImageUtils.loadTexture( textureName );
}
...
function getMeshPhongMaterial(texture, color){
    return new THREE.MeshPhongMaterial({transparent: true, map: texture, color: color});
...
function getSphereMesh(radius, position, rotation, scaling, color) {
    // the column on the frustum
    var geometry = new THREE.SphereGeometry(radius, 30, 30, 30, 30, 10);
...
function getRandomFloatNumberFromInterval() {
    var min = 0.0005,
        max = 0.005,
        highlightedNumber= min + (max-min) * Math.random();
